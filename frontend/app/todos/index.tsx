...
export default function TodoLists() {
  ...
  const load = useCallback(async () => {
    try {
      setLists(await api("/todos/lists"));
    } catch {}
  }, []);
  ...
  const add = async () => {
    if (!name.trim()) return;
    await api("/todos/lists", { method: "POST", body: { name: name.trim() } });
    setName("");
    setAdding(false);
    load();
  };
  ...
  const del = async (id: string) => {
    ...
    await api(`/todos/lists/${id}`, { method: "DELETE" });
  };

  return (
    <View style={[styles.container, { backgroundColor: c.surfaceSecondary, paddingTop: insets.top }]}>
      <View style={styles.header}>
        <Pressable onPress={() => router.back()} hitSlop={12} testID="todos-back">
          <Ionicons name="chevron-back" size={26} color={c.onSurface} />
        </Pressable>
        <AppText family="display" weight="bold" size={20}>
          To-Do Lists
        </AppText>
        <Pressable onPress={() => setAdding((a) => !a)} hitSlop={12} testID="toggle-add-todolist">
      ...
      <KeyboardAwareScrollView contentContainerStyle={{ padding: spacing.lg, paddingBottom: 120 }} bottomOffset={20} showsVerticalScrollIndicator={false}>
        {adding ? (
          <View style={[styles.addCard, { backgroundColor: c.surface, borderColor: c.border }, shadow(1)]}>
            <TextField placeholder="List name, e.g. Weekend Plans" value={name} onChangeText={setName} testID="todolist-name-input" />
            <Button label="Create List" onPress={add} style={{ marginTop: spacing.md }} testID="create-todolist-btn" />
          </View>
        ) : null}
        ...
        {lists.length === 0 ? (
          <View style={styles.empty}>
            ...
            <AppText size={13} color={c.onSurfaceTertiary} center style={{ marginTop: 4 }}>
              Create a shared list the whole family can update
            </AppText>
          </View>
        ) : (
          lists.map((l) => {
            ...
            return (
              <Pressable
                key={l.list_id}
                onPress={() => router.push(`/todos/${l.list_id}?name=${encodeURIComponent(l.name)}`)}
                style={[styles.listCard, { backgroundColor: c.surface, borderColor: c.border }, shadow(1)]}
                testID={`todo-list-${l.list_id}`}
              >
                <View style={[styles.listIcon, { backgroundColor: c.info + "22" }]}>
                  <Ionicons name="checkbox-outline" size={22} color={c.info} />
                </View>
                <View style={{ flex: 1 }}>
                  ...
                  <View style={[styles.progressTrack, { backgroundColor: c.surfaceTertiary }]}>
                    <View style={[styles.progressFill, { width: `${pct * 100}%`, backgroundColor: c.info }]} />
                  </View>
                </View>
                <Pressable onPress={() => del(l.list_id)} hitSlop={8} testID={`del-todolist-${l.list_id}`}>
